...
export function renderNotificacionesView(usuarioActual) {
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; flex-wrap: wrap; gap: 10px;">
      <div>
        <h1 style="margin: 0; font-size: 22px; font-weight: 800; color: var(--primary); display: flex; align-items: center; gap: 8px;">
          <span style="display: flex; align-items: center; color: var(--primary);">${icons.bell || '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9"></path><path d="M13.73 21a2 2 0 0 1-3.46 0"></path></svg>'}</span>
  ...
  async function cargarNotificaciones() {
    ...
    // Comprobar si ya sigues a alguno de los emisores para adaptar los botones
    const { data: misSeguidos } = await supabase
      .from('seguidores')
      .select('seguido_id, estado')
      .eq('seguidor_id', usuarioActual.id)
      .in('seguido_id', emisorIds);

    const mapaSiguiendo = {};
    (misSeguidos || []).forEach(s => { mapaSiguiendo[s.seguido_id] = s.estado; });
    ...
    lista.innerHTML = notifsUnicas.map(n => {
      ...
      const yaLoSigo = mapaSiguiendo[n.emisor_id] === 'aceptado';
      ...
      return `
        <div style="display: flex; align-items: center; justify-content: space-between; padding: 12px 14px; background: var(--input-bg); border-radius: 12px; border: 1px solid var(--border); flex-wrap: wrap; gap: 10px;">
          <div style="display: flex; align-items: center; gap: 10px;">
            <div style="width: 36px; height: 36px; border-radius: 50%; background: var(--primary-light); color: var(--primary); display: flex; align-items: center; justify-content: center; font-weight: 800; font-size: 13px; overflow: hidden; flex-shrink: 0;">
    ...
    // MANEJADOR: ACEPTAR Y SEGUIR TAMBIÉN
    lista.querySelectorAll('.btn-aceptar-y-seguir').forEach(btn => {
      btn.addEventListener('click', async () => {
        ...
        try {
          ...
          // 3. Borrar la notificación respondida
          await supabase.from('notificaciones').delete().eq('id', notifId);

          // 4. Enviar notificación al emisor original confirmando la aceptación
